...
function getContents(string $filepath1, string $filepath2)
{
    ...
    switch ($normalizedYamlFormat) {
        case 'json':
            $file1Content = json_decode(strval($filepath1), true);
            $file2Content = json_decode(strval($filepath2), true);
            return [$file1Content, $file2Content];
        case 'yaml':
            $file1Content = Yaml::parse(strval($filepath1), Yaml::PARSE_OBJECT_FOR_MAP);
            $file2Content = Yaml::parse(strval($filepath2), Yaml::PARSE_OBJECT_FOR_MAP);
